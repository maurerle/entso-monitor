...
class EntsoeParquet(EntsoeDataManager):
    ...
    def capacity(self, country: str):
        cap = self.spark.read.parquet(
            '{}/{}/query_installed_generation_capacity'.format(self.folder, country))
        df = cap.withColumn("time", date_trunc('year', "time")
                            ).distinct().sort('time').toPandas()
        df.index = df['time']
        del df['time']
        del df['year']
        del df['month']
    ...
    def load(self, country: str, filt: Filter):
        load = self.spark.read.parquet(
            '{}/{}/query_load'.format(self.folder, country))
    ...
    def generation(self, country: str, filt: Filter):
        gen = self.spark.read.parquet(
            '{}/{}/query_generation'.format(self.folder, country))
        timeString = '"{}" < time and time < "{}"'.format(
            filt.begin.strftime("%Y-%m-%d"), filt.end.strftime("%Y-%m-%d"))
    ...
    def consumption(self, country: str, filt: Filter):
        pumpSto = self.spark.read.parquet(
            '{}/{}/query_generation'.format(self.folder, country))
        timeString = '"{}" < time and time < "{}"'.format(
            filt.begin.strftime("%Y-%m-%d"), filt.end.strftime("%Y-%m-%d"))
        cols = list(filter(lambda x: x.endswith(
    ...
    def crossborderFlows(self, country: str, filt: Filter):
        timeString = '"{}" < time and time < "{}"'.format(
            filt.begin.strftime("%Y-%m-%d"), filt.end.strftime("%Y-%m-%d"))
        crossborder = self.spark.read.parquet(
            '{}/query_crossborder_flows'.format(self.folder))
        crossborder = crossborder.filter(timeString).withColumn(
            "group", date_trunc(filt.groupby, "time"))

        relList = map(lambda x: x.split('.'), crossborder.columns)
        filteredRelations = filter(lambda x: x.count(country) > 0, relList)
        columns = list(map(lambda x: '{}.{}'.format(
            x[0], x[1]), filteredRelations))
...
if __name__ == "__main__":
    ...
    load = par.load('DE', Filter(datetime(2019, 1, 1),
                    datetime(2019, 1, 2), 'hour'))
    generation = par.generation('DE', Filter(
        datetime(2019, 1, 1), datetime(2019, 1, 2), 'hour'))
    gen = generation.melt(
